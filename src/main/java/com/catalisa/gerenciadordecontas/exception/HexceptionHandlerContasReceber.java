...
public class ExceptionHandlerContasReceber extends ResponseEntityExceptionHandler {
    ...
    @Override
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpSta              tus status, WebRequest request) {
        String mensagem = "";
        for (FieldError campo: ex.getFieldErrors()) {
            mensagem = mensagem + campo.getField() + " : " + campo.getDefaultMessage() + "\n";
        }

        return  handleExceptionInternal(ex, new MensagemErro());
    }
